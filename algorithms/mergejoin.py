...
def mergejoin(optimalcost, hashcost, mergecost, nestloopcost, indexnestloopcost):
    ...
    if nestloopcost != math.inf:
        if nestloopcost == -1:
            ...
            annotation += ". NESTED LOOP JOIN would have costed: " + str("%.2f" % nestloopcost) + " which costs " + str(scale_nested_loop) \
                      + " times more"
    else:
